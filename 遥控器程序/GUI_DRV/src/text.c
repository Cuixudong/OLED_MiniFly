...
設取消颜色测试姿态感应陀螺闪烁警\
报电筒呼吸水顏測試態應閃爍報電激\
器顯显数字据图形三维效果亮方向圍數據圖屏幕于於打正反"
};
             
//char * st = "控制模式飞行速度翻滚使能摇杆校准匹配四轴语言选择重置退出扩展块飛滾搖桿準軸語選擇擴塊定高手动点動點无头有無頭低中开启关闭開啟關閉简体文繁未发现！灯环摄像光流發現燈環攝两个一圈兩個按下步保持回完成提示：已…通信失败请先遥和再。敗請遙功确恢复为默认设？確復為認設取消颜色测试姿态感应陀螺闪烁警报电筒呼吸水顏測試態應閃爍報電激器顯显数字据图形三维效果亮方向圍數據圖屏幕于於";
