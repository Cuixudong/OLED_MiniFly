...
void configParamInit(void)
{
    ...
    /* 读取配置参数 */
    STMFLASH_Read(CONFIG_PARAM_ADDR,(uint16_t*)&configParam,sizeof(configParam)/2);
...
void configParamTask(void* param)
{
    ...
    while(1)
    {
        ...
        {
            count = 0;
            STMFLASH_Write(CONFIG_PARAM_ADDR,(uint16_t*)&configParam,sizeof(configParam)/2);
        }
    }
}
