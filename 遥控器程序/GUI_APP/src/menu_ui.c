...
void gotoNextMenu(void)
{
    if(CurItem->childrenMenu != NULL)
    {
        ...
        if((*(CurItem->title) >= '1') && ((*(CurItem->title) <= '9')))
        {
            MenuWindow.title = CurItem->title +2;
        }
        else
        {
            MenuWindow.title = CurItem->title;
        }
        isChangeMenu = true;
    }
...
void gotoLastMenu(void)
{
    if(CurItem->parentMenu != NULL)
    {
        ...
        {
//            CurMenu = &CurItem->parentMenu[selected];
//            if((*(CurMenu->title) >= '1') && ((*(CurMenu->title) <= '9')))
//            {
//                MenuWindow.title = CurMenu->title +2;
//            }
//            else
//            {
//                MenuWindow.title = CurMenu->title;
//            }
            MenuWindow.title = mainMenu[selected].title + 2;
        }
