...
#define  LOW_POWER  3.f
#define  FULL_POWER 4.f
...
extern uint8_t rssi;//四轴返回的无线信号值
...
void main_ui(void)
{
    ...
    char st[8];
    sprintf(st,"%08x",configParam.radio.addressHigh);
    oled_showString(40,0,(uint8_t*)st,6,12);
    sprintf(st,"%08x",configParam.radio.addressLow);
    oled_showString(40,12,(uint8_t*)st,6,12);
...
static uint8_t reReceiveTimes = 3;	/*重接收次数*/

/*四轴MSG处理*/
void miniFlyMsgACKProcess(atkp_t* p)
{
    msg = *(MiniFlyMsg_t*)(p->data+1);
    if(msg.mpu_selfTest==false)
    {
